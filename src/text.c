...
--*/
{
   ...
   //
   fpMsg = fopen("e:\\data\\pal\\m.msg", "rb");
   //fpMsg = fopen("m.msg", "rb");
   if (fpMsg == NULL)
   {
      return -2;
   }
   fpWord = fopen("e:\\data\\pal\\word.dat", "rb");
...
--*/
{
   ...
   while (TRUE)
   {
      ...
      //PAL_ProcessEvent();
      //if (g_InputState.dwKeyPress & (kKeySearch | kKeyMenu)) //改为按任意键
         ...
         while (lpszText != NULL && *lpszText != '\0')
         {
            ...
            default:
               ...
               if (!g_TextLib.fUserSkip)
               {
                  ...
                  if (g_InputState.dwKeyPress ) // any key to skip	  
                  {
                     //
